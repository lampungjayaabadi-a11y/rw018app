...
export const LaporanKejadianView: React.FC<LaporanKejadianViewProps> = ({
  ...
  // PDF Generation & Web Share Handler for Official A4 Document
  const generateAndSharePdf = async (
    lap: LaporanKejadian,
    mode: 'share' | 'download' = 'share'
  ) => {
    setIsGeneratingPdf(true);
    setPdfProgressMsg('Menyiapkan dokumen resmi format kertas A4...');
    ...
    try {
      ...
      setPdfProgressMsg('Mengonversi dokumen ke berkas PDF A4 resolusi tinggi...');
      ...
      const cleanNomor = (lap.nomorLaporan || 'LAP').replace(/[^a-zA-Z0-9_-]/g, '_');
      ...
      if (mode === 'download') {
        pdf.save(filename);
        setPdfSuccessToast(`Berkas PDF resmi ${filename} berhasil disimpan di penyimpanan perangkat.`);
        setTimeout(() => setPdfSuccessToast(null), 4000);
        return;
      }

      // Check if browser Web Share API supports file sharing (Android, iOS, Modern Browsers)
      let shareCompleted = false;
      if (typeof navigator !== 'undefined' && navigator.canShare && navigator.canShare({ files: [file] })) {
        setPdfProgressMsg('Membuka menu aplikasi (WhatsApp, Telegram, Email, atau Penyimpanan)...');
        try {
          await navigator.share({
            title: `Laporan Kejadian RW 018 - ${lap.nomorLaporan}`,
            text: `Berikut berkas asli dokumen PDF Laporan Kejadian RW 018 Kampung Banten (Format Kertas A4 Siap Cetak). Nomor: ${lap.nomorLaporan}`,
            files: [file],
          });
          shareCompleted = true;
          setPdfSuccessToast('Berkas asli PDF berhasil dikirim ke aplikasi pilihan via Web Share API.');
          setTimeout(() => setPdfSuccessToast(null), 4500);
        } catch (shareErr: any) {
          if (shareErr?.name === 'AbortError') {
            // Pengguna menutup dialog share sheet bawaan perangkat
            return;
          }
      ...
      if (!shareCompleted) {
        // Fallback: unduh berkas PDF ke perangkat dan buka modal pilihan kirim
        pdf.save(filename);
        setShareLaporan(lap);
        setPdfSuccessToast(`Berkas PDF resmi (${filename}) tersimpan di perangkat. Silakan pilih aplikasi pengiriman WhatsApp, Telegram, atau Email.`);
  ...
  const handleShareToWhatsApp = async (lap: LaporanKejadian) => {
    await generateAndSharePdf(lap, 'download');
    const waText = encodeURIComponent(formatLaporanWhatsApp(lap));
    window.open(`https://api.whatsapp.com/send?text=${waText}`, '_blank');
  };

  const handleShareToTelegram = async (lap: LaporanKejadian) => {
    await generateAndSharePdf(lap, 'download');
    const tgText = encodeURIComponent(
      `*LAPORAN KEJADIAN RESMI RW 018 KAMPUNG BANTEN*\nNomor: ${lap.nomorLaporan}\nJenis: ${lap.jenisKejadian}\nLokasi: ${lap.lokasiKejadian} (RT ${lap.rt} RW 018)\n\n(Berkas asli dokumen PDF format A4 siap cetak telah diunduh, silakan lampirkan ke pesan ini)`
    );
    window.open(`https://t.me/share/url?text=${tgText}`, '_blank');
  };

  const handleShareToEmail = async (lap: LaporanKejadian) => {
    await generateAndSharePdf(lap, 'download');
    const subject = encodeURIComponent(`Laporan Kejadian Resmi RW 018 - Nomor: ${lap.nomorLaporan}`);
    const body = encodeURIComponent(
      `Kepada Yth. Pihak Berwenang / Instansi Terkait,\n\nBersama ini kami lampirkan berkas dokumen resmi Laporan Kejadian RW 018 Kampung Banten format A4 siap cetak:\n\nNomor: ${lap.nomorLaporan}\nJenis Kejadian: ${lap.jenisKejadian}\nHari/Tanggal: ${lap.hariKejadian}, ${formatTanggalIndonesia(lap.tanggalKejadian)} - ${lap.waktuKejadian}\nLokasi: ${lap.lokasiKejadian} (RT ${lap.rt} RW 018)\n\n(Berkas PDF resmi telah diunduh di perangkat. Silakan lampirkan file PDF tersebut pada pesan ini).\n\nHormat kami,\nPengurus RW 018 Kampung Banten\nKelurahan Iringmulyo, Kecamatan Metro Timur`
    );
  ...
  const filteredList = useMemo(() => {
    return laporanList.filter((lap) => {
      ...
      const matchQuery =
        ...
        lap.korban.some((k) => k.nama.toLowerCase().includes(q)) ||
      ...
      return matchQuery && matchJenis && matchRt && matchStatus;
    });
  }, [laporanList, searchQuery, filterJenis, filterRt, filterStatus]);

  // Statistics Summary
  const stats = useMemo(() => {
    const total = laporanList.length;
    const selesai = laporanList.filter((l) => l.status === 'Selesai').length;
    const dilaporkan = laporanList.filter((l) => l.status === 'Sudah Dilaporkan' || l.status === 'Sudah Dibuat').length;
    const totalKerugian = laporanList.reduce((acc, curr) => acc + (curr.nilaiKerugian || 0), 0);
    return { total, selesai, dilaporkan, totalKerugian };
  }, [laporanList]);
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-3.5">
              ...
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase block">Total Laporan</span>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-3.5">
              ...
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase block">Tuntas / Selesai</span>
                <span className="text-2xl font-black text-emerald-700">{stats.selesai}</span>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-3.5">
              ...
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase block">Proses / Dilaporkan</span>
                <span className="text-2xl font-black text-amber-700">{stats.dilaporkan}</span>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-3.5">
              ...
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase block">Total Kerugian</span>
                <span className="text-lg font-black text-rose-700">{formatRupiah(stats.totalKerugian)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Cari nomor laporan, nama korban, lokasi, tanggal, jenis kejadian..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-hidden focus:ring-2 focus:ring-emerald-600"
                />
                {searchQuery && (
                  <button
              ...
              <div className="flex flex-wrap items-center gap-2">
                <select
                  ...
                  className="px-3 py-2.5 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 bg-white"
                >
                  <option value="all">Semua Kejadian</option>
                  {DAFTAR_JENIS_KEJADIAN.map((j) => (
          ...
          {filteredList.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
              ...
              <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
                {searchQuery || filterJenis !== 'all' || filterRt !== 'all'
                  ...
                  >
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-2 mb-3">
                        <div className="flex items-center gap-2.5">
                          <div className={`p-2.5 rounded-xl border ${jenisObj.color}`}>
                          ...
                          <div>
                            ...
                            <span className="text-[11px] text-slate-500 flex items-center gap-1">
                              <Calendar className="w-3 h-3" /> {lap.hariKejadian}, {formatTanggalIndonesia(lap.tanggalKejadian)}
                            </span>
                          </div>
                        </div>
                        ...
                        <span
                          className={`text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full border ${
                            lap.status === 'Selesai'
                              ...
                              : lap.status === 'Sudah Dilaporkan'
                              ? 'bg-blue-50 text-blue-700 border-blue-200'
                              : lap.status === 'Sudah Dicetak'
                    ...
                    <div className="pt-2 flex items-center justify-between gap-2 border-t border-slate-100 mt-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          onClick={() => setPreviewLaporan(lap)}
                          className="px-2.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg text-xs font-semibold flex items-center gap-1 transition"
                        ...
                        <button
                          ...
                          title="Buka Menu Bagikan Hasil Cetak File PDF (A4)"
                        >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Sudah Dibuat">Sudah Dibuat (Resmi)</option>
                    <option value="Sudah Dicetak">Sudah Dicetak</option>
                    <option value="Sudah Dilaporkan">Sudah Dilaporkan</option>
      ...
      {shareLaporan && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-xl w-full p-5 sm:p-6 shadow-2xl space-y-4 border border-slate-200 animate-in fade-in zoom-in duration-150 my-auto">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700 font-medium">
                    Berkas File PDF Format Surat Resmi Kertas A4 — Siap Cetak ke Printer
                  </p>
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-blue-50/90 via-slate-50 to-indigo-50/60 border-2 border-blue-200/90 space-y-3.5 shadow-xs">
              ...
              <div className="bg-blue-50/90 p-3 rounded-xl border border-blue-200/80 flex items-start gap-2.5">
                ...
                <div className="text-[11px] text-blue-950 leading-relaxed space-y-1">
                  <p className="m-0">
                    <strong>Dokumen Berkas Asli PDF (Kertas A4):</strong> Berkas yang dikirimkan adalah <strong>file fisik dokumen PDF resmi</strong> tata naskah dinas pemerintah (Kop RW 018 Kampung Banten, kronologis kejadian, lampiran dokumentasi, serta tanda tangan Bhabinkamtibmas Aipda Evodius & Ketua RW 018).
                  </p>
                  <p className="m-0 text-blue-800 font-medium">
                    <strong>Web Share Document API:</strong> Sistem langsung mengirimkan berkas asli dokumen PDF ke lembar pilihan aplikasi perangkat Anda (WhatsApp, Telegram, Gmail/Email, Drive, atau Penyimpanan Perangkat).
                  </p>
                </div>
              </div>

              {/* Tombol Utama: Bagikan Berkas Asli PDF via Web Share Document API */}
              <div className="space-y-3 pt-1">
                <button
                  type="button"
                  onClick={() => generateAndSharePdf(shareLaporan, 'share')}
                  disabled={isGeneratingPdf}
                  className="w-full py-3.5 px-4 bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 hover:from-blue-700 hover:to-indigo-800 active:scale-[0.99] text-white rounded-xl text-left flex items-center gap-3 transition shadow-md hover:shadow-lg disabled:opacity-50 cursor-pointer"
                >
                  ...
                  <div className="grow min-w-0">
                    <div className="text-xs sm:text-sm font-extrabold flex items-center gap-1.5 flex-wrap">
                      <span>Bagikan Berkas File PDF (A4) ke WhatsApp / Kontak</span>
                      <span className="px-1.5 py-0.2 text-[9px] bg-white/25 rounded uppercase tracking-wider font-bold">
                        Web Share API
                      </span>
                    </div>
                    <div className="text-[10px] sm:text-[11px] text-blue-100 font-normal leading-tight mt-0.5">
                      Mengirimkan berkas asli dokumen PDF — Buka Aplikasi Pilihan (WhatsApp, Telegram, Email, atau Penyimpanan Perangkat)
                    </div>
                  </div>
                </button>

                {/* Grid Pilihan Aplikasi Tujuan Cepat */}
                <div className="pt-1">
                  <span className="text-[11px] font-bold text-slate-700 mb-2 flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-blue-600" />
                    Pilihan Aplikasi Langsung (Kirim & Simpan Berkas Asli PDF):
                  </span>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-1.5">
                    ...
                    <button
                      ...
                      title="Unduh Berkas PDF & Buka WhatsApp"
                    >
                      ...
                      <span className="text-[9px] text-emerald-700 leading-none">Pesan & Berkas</span>
                    </button>
                    ...
                    <button
                      ...
                      title="Unduh Berkas PDF & Buka Telegram"
                    >
                      ...
                      <span className="text-[9px] text-sky-700 leading-none">Kirim Berkas</span>
                    </button>
                    ...
                    <button
                      ...
                      title="Unduh Berkas PDF & Buka Email"
                    >
                      ...
                      <span className="text-[9px] text-amber-800 leading-none">Surat Dinas</span>
                    </button>
                    ...
                    <button
                      ...
                      disabled={isGeneratingPdf}
                      className="p-2.5 bg-slate-50/90 hover:bg-slate-100 active:bg-slate-200 border border-slate-300 rounded-xl text-slate-800 flex flex-col items-center justify-center gap-1 transition cursor-pointer group text-center shadow-2xs"
                      title="Simpan Berkas PDF ke Penyimpanan Perangkat"
                    >
                      ...
                      <span className="text-[11px] font-bold">Penyimpanan</span>
                      <span className="text-[9px] text-slate-600 leading-none">Simpan ke HP/PC</span>
                    </button>
                  </div>
                </div>
            ...
            {/* PANDUAN PENGIRIMAN & RINGKASAN PENGANTAR (OPSIONAL) */}
            <div className="space-y-2 pt-1">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                  <Info className="w-3.5 h-3.5 text-slate-500" />
                  Petunjuk Pengiriman Dokumen PDF via WhatsApp:
                </span>
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(formatLaporanWhatsApp(shareLaporan));
                    setShareCopied(true);
                ...
                >
                  ...
                  {shareCopied ? 'Tersalin!' : 'Salin Keterangan Pengantar'}
                </button>
              </div>

              <div className="text-[11px] text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1.5 leading-relaxed">
                <p className="m-0">
                  <strong>• Di Handphone / Tablet:</strong> Klik tombol <em>"Bagikan Berkas File PDF (A4)"</em> di atas, lalu pilih aplikasi <strong>WhatsApp</strong> ➔ pilih Kontak/Grup tujuan. Berkas PDF resmi A4 akan langsung terlampir otomatis.
                </p>
                <p className="m-0">
                  <strong>• Di Komputer / WhatsApp Web:</strong> Klik tombol <em>"Unduh File PDF Resmi (A4)"</em>, lalu pada WhatsApp Web buka chat tujuan, klik ikon <strong>Lampiran (📎)</strong> ➔ pilih <strong>Dokumen</strong> ➔ pilih file PDF yang telah diunduh.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100">
              ...
              >
                <Eye className="w-4 h-4" /> Buka Pratinjau Lembar Surat A4
              </button>
          ...
          <div className="bg-white rounded-2xl max-w-3xl w-full shadow-2xl border border-slate-300 overflow-hidden flex flex-col my-auto print:border-none print:shadow-none print:rounded-none">
            ...
            <div className="bg-slate-900 text-white p-4 flex items-center justify-between print:hidden">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-sm">Dokumen Resmi Laporan Kejadian RW 018</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => window.print()}
                  className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg flex items-center gap-1.5 transition shadow-sm cursor-pointer"
                  title="Cetak langsung ke printer atau simpan PDF melalui browser"
                >
                  <Printer className="w-4 h-4" /> Cetak ke Printer
                </button>
                <button
                  onClick={() => generateAndSharePdf(previewLaporan, 'download')}
                  disabled={isGeneratingPdf}
                  className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white text-xs font-semibold rounded-lg flex items-center gap-1.5 transition cursor-pointer"
                  title="Unduh berkas PDF format resmi kertas A4"
                >
                  <FileDown className="w-4 h-4" /> Unduh PDF (A4)
                </button>
                <button
                  onClick={() => generateAndSharePdf(previewLaporan, 'share')}
                  disabled={isGeneratingPdf}
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg flex items-center gap-1.5 transition cursor-pointer shadow-xs"
                  title="Bagikan berkas file PDF resmi A4"
                >
            ...
            {/* Sub-bar Info Bhabinkamtibmas & RT 039 (Hidden on Print) */}
            <div className="bg-slate-800 text-slate-200 px-4 py-2 text-xs flex flex-wrap items-center justify-between gap-3 border-t border-slate-700 print:hidden">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-slate-400 text-[11px]">Bhabinkamtibmas:</span>
                <span className="px-2 py-0.5 rounded bg-emerald-900/70 border border-emerald-500/40 text-emerald-300 font-bold text-[11px]">
                  {namaBhabinkamtibmas} (NRP. {nrpBhabinkamtibmas})
                </span>
                <span className="text-slate-400 text-[11px] ml-2">Alamat Sekretariat:</span>
                <span className="px-2 py-0.5 rounded bg-slate-700 text-slate-200 font-semibold text-[11px]">
                  Jl. Pala Nomor 1 RT 039 RW 018
                </span>
              </div>
              <div className="flex items-center gap-3">
                <label className="inline-flex items-center gap-1.5 cursor-pointer text-[11px] hover:text-white">
                  ...
                  />
                  <span>Sertakan Kolom Babinsa ({namaBabinsa})</span>
                </label>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full text-center space-y-4 border border-slate-200 animate-in fade-in zoom-in duration-200">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 mt-1">
                {pdfProgressMsg || 'Menyiapkan berkas dokumen resmi RW 018...'}
              </p>
            </div>
