...
export const Header: React.FC<HeaderProps> = ({
  ...
  const headerRwTitle = profile?.namaRw
    ? profile.namaRw.replace(/RUKUN WARGA\s*/gi, 'RW ')
    : 'RW 018';

  const handleInstallPWA = async () => {
    setIsUserMenuOpen(false);
    const prompted = await install();
    if (!prompted && onOpenAndroidModal) {
      onOpenAndroidModal();
    }
  };
      ...
      <div className="px-3.5 sm:px-5 py-2.5">
        <div className="flex items-center justify-between gap-2.5">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0 select-none">
              {/* Logo with gold border ring - Toggle Fullscreen on click */}
              <button
                type="button"
                id="btn-header-logo-fullscreen"
                onClick={(e) => {
                  e.stopPropagation();
                  toggleFullscreen();
                }}
                title={
                  isFullscreen
                    ? 'Klik Logo: Keluar dari Layar Penuh (Exit Fullscreen)'
                    : 'Klik Logo: Aktifkan Layar Penuh (Fullscreen)'
                }
                aria-label={isFullscreen ? 'Keluar Fullscreen' : 'Aktifkan Layar Penuh Fullscreen'}
                className="relative w-11 h-11 sm:w-12 sm:h-12 rounded-2xl overflow-hidden bg-slate-950 border-2 border-amber-400 shadow-md shrink-0 flex items-center justify-center ring-2 ring-amber-400/40 hover:ring-amber-300 hover:scale-105 active:scale-95 transition-all duration-200 p-0.5 cursor-pointer group/logo"
              >
                <img
                ...
                />
                {/* Fullscreen status indicator on corner */}
                <span className="absolute bottom-0 right-0 p-0.5 rounded-tl-md bg-slate-950/85 text-amber-300 border-t border-l border-amber-400/50 shadow-xs flex items-center justify-center group-hover/logo:scale-110 transition-transform">
                  {isFullscreen ? (
                    <Minimize2 className="w-2.5 h-2.5 stroke-[2.5]" />
                  ) : (
                    <Maximize2 className="w-2.5 h-2.5 stroke-[2.5]" />
                  )}
                </span>
              </button>

              {/* Title & Subtitle Info (Klik untuk ke Dashboard) */}
              <div
              ...
              >
                <div className="flex items-center gap-1.5 flex-wrap">
                  <h1 className="text-base sm:text-lg font-black leading-tight text-white tracking-tight truncate drop-shadow-xs group-hover/title:text-amber-200 transition-colors">
                    {headerRwTitle}
                  </h1>
                </div>

                <div className="flex items-center gap-1 text-[10px] sm:text-xs text-emerald-200/90 font-medium truncate mt-0.5">
                  <MapPin className="w-2.5 h-2.5 text-amber-300 shrink-0" />
                  <span className="truncate">
                    {profile.kelurahan || 'Iringmulyo'}, {profile.kotaKab || 'Kota Metro'}
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Right: Actions Cluster (Emergency SOS, Notification Bell, Theme Switcher, User Account Capsule) */}
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            {onOpenEmergencyModal && (
              <button
                ...
                className="relative flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-tr from-rose-700 via-red-600 to-rose-500 hover:from-rose-600 hover:to-red-500 active:from-rose-800 text-white shadow-md shadow-rose-600/50 border border-rose-300 ring-2 ring-rose-400/80 animate-pulse transition-all active:scale-90 group shrink-0 cursor-pointer"
              >
                {/* Flashing Beacon Dot */}
            ...
            <button
              ...
              className="relative p-2 rounded-xl bg-emerald-800/80 hover:bg-emerald-700 active:bg-emerald-900 text-emerald-100 border border-emerald-600/50 shadow-xs transition-all active:scale-95 flex items-center justify-center cursor-pointer"
            ...
            <div className="relative">
              {currentUser ? (
                <button
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-2.5 py-1 rounded-xl bg-emerald-800/90 hover:bg-emerald-700/90 active:bg-emerald-900 border border-emerald-600/70 shadow-xs transition-all active:scale-95 cursor-pointer"
                  title={`User Aktif: ${currentUser.nama} (${currentUser.roleLabel})`}
                  aria-label="Buka Menu Akun & PWA"
                >
                  <div className="relative w-7 h-7 sm:w-8 sm:h-8 rounded-full overflow-hidden border-2 border-amber-300/90 bg-slate-900 flex items-center justify-center font-black text-[11px] shadow-2xs shrink-0">
                    <img
                    ...
                    />
                    <span className="absolute bottom-0 right-0 w-2 h-2 rounded-full bg-emerald-400 border border-emerald-950"></span>
                  </div>
                  <span className="text-xs sm:text-sm font-black truncate text-white max-w-[80px] sm:max-w-[120px]" title={currentUser.nama}>
                ...
                <button
                  id="btn-header-login"
                  onClick={onOpenLogin}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-400 hover:bg-amber-300 active:bg-amber-500 text-emerald-950 text-xs font-black transition-all shadow-md active:scale-95 cursor-pointer"
                >
              ...
              {isUserMenuOpen && currentUser && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 z-50 text-slate-800 dark:text-slate-100 p-2.5 text-xs divide-y divide-slate-100 dark:divide-slate-800 animate-in fade-in slide-in-from-top-2 duration-150">
                    ...
                    {/* Unified Install PWA Action Card (1 Saja Terintegrasi dengan Profil Akun Aktif) */}
                    <div className="py-1.5">
                      <button
                        id="btn-menu-install-pwa"
                        onClick={handleInstallPWA}
                        className="w-full text-left p-2.5 rounded-xl bg-gradient-to-r from-emerald-600 via-emerald-700 to-teal-800 hover:from-emerald-500 hover:to-teal-700 text-white font-bold transition-all shadow-md flex items-center justify-between group cursor-pointer border border-emerald-400/40"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center shrink-0 shadow-xs group-hover:scale-105 transition-transform">
                            <Smartphone className="w-4 h-4 text-amber-300" />
                          </div>
                          <div className="min-w-0">
                            <div className="text-xs font-black leading-tight truncate flex items-center gap-1">
                              <span>Install Aplikasi RW</span>
                              <span className="px-1 py-0.2 rounded text-[8px] bg-amber-400 text-emerald-950 font-black uppercase">
                                PWA
                              </span>
                            </div>
                            <div className="text-[10px] text-emerald-100/90 font-medium leading-tight truncate">
                              {isInstalled ? 'Aplikasi sudah terpasang' : 'Pasang di Layar Utama HP Android'}
                            </div>
                          </div>
                        </div>
                        <span className="text-[10px] px-2 py-1 rounded-lg bg-amber-400 hover:bg-amber-300 text-emerald-950 font-extrabold shadow-xs shrink-0 ml-1.5">
                          {isInstalled ? 'Buka' : 'Install'}
                        </span>
                      </button>
                    </div>
