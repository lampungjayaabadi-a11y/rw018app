...
export const BansosReportModal: React.FC<BansosReportModalProps> = ({
  ...
  const handleDownloadPdf = async () => {
    ...
    const filename = `Laporan-Bansos-RW018-${dateStr}.pdf`;

    await downloadElementAsPdf('bansos-report-print-sheet', {
      filename,
      orientation: 'landscape',
      title: 'Laporan Rekapitulasi Bantuan Sosial RW 018',
      author: 'Pengurus RW 018 Kampung Banten',
      subject: 'Rekapitulasi Penyaluran Bantuan Sosial Warga RW 018',
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/75 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 overflow-y-auto print:p-0 print:bg-white print:static">
      <div className="bg-white rounded-3xl w-full max-w-6xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh] print:max-h-none print:shadow-none print:border-none print:rounded-none">
        ...
        {/* Header Dialog (Hidden on Print) - Ramping & Nyaman Android */}
        <div className="px-3 py-2 sm:px-4 sm:py-3 bg-gradient-to-r from-slate-900 via-slate-800 to-amber-950 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 shrink-0 print:hidden shadow-sm">
          {/* Sisi Kiri: Kembali, Judul Ramping, Badge */}
          <div className="flex items-center justify-between sm:justify-start gap-2 min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              <button
                type="button"
                onClick={onClose}
                className="px-2.5 py-1.5 bg-white/10 hover:bg-white/20 text-slate-200 hover:text-white rounded-xl text-xs font-semibold flex items-center gap-1 transition-all border border-white/20 shadow-2xs shrink-0 cursor-pointer active:scale-95"
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-slate-300 truncate hidden sm:block">
                  Rekapitulasi penyaluran bansos Kel. {profile.kelurahan}, {profile.kotaKab}
                </p>
              </div>
            </div>

            {/* Tombol Tutup X di Layar Mobile */}
            <button
              type="button"
              onClick={onClose}
          ...
          {/* Susunan Tombol Menu: Rata Kiri & Kanan di Android, Ramping & Nyaman */}
          <div className="flex items-center justify-between sm:justify-end gap-1.5 sm:gap-2 w-full sm:w-auto shrink-0 pt-1 sm:pt-0 border-t border-slate-800/80 sm:border-t-0">
            {/* Tombol Utama: 🖨️ CETAK */}
            <button
              id="btn-cetak-bansos"
              type="button"
              onClick={handlePrint}
              className="flex-1 sm:flex-initial px-3 py-1.5 bg-slate-800 hover:bg-slate-700 active:bg-slate-950 text-white text-xs font-bold rounded-xl border border-slate-700 shadow-2xs transition-all cursor-pointer active:scale-95 flex items-center justify-center gap-1.5"
            ...
            <button
              id="btn-download-pdf-bansos"
              type="button"
              onClick={handleDownloadPdf}
              disabled={isGeneratingPdf}
              className="flex-1 sm:flex-initial px-3.5 py-1.5 bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-slate-950 text-xs font-black rounded-xl shadow-md transition-all cursor-pointer active:scale-95 disabled:opacity-50 flex items-center justify-center gap-1.5"
              title="Download Rekap Bansos Menjadi File PDF Kertas A4"
            >
            ...
            <button
              ...
              className="px-2.5 sm:px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-bold rounded-xl shadow-xs transition-all cursor-pointer shrink-0 flex items-center gap-1 active:scale-95"
            ...
            {/* Tombol Tutup X di Layar Desktop */}
            <button
              type="button"
              onClick={onClose}
              className="hidden sm:flex p-1.5 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors cursor-pointer shrink-0 ml-1"
              title="Tutup Laporan"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div
          id="bansos-report-print-sheet"
          className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 text-slate-800 print:overflow-visible print:p-0"
        >
          
          {/* Printable Official Letterhead (Kop Surat) */}
          <div className="border-b-2 border-slate-800 pb-3 sm:pb-4 text-center">
            <div className="flex items-center justify-center gap-2.5 sm:gap-3 mb-1">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-amber-600 text-white flex items-center justify-center font-black text-base sm:text-xl shadow-xs print:border print:border-black shrink-0">
                RW
              </div>
              <div className="text-left">
                <h1 className="text-sm sm:text-base font-black uppercase tracking-wide text-slate-900 leading-tight">
            ...
            <div className="mt-2.5 pt-1.5 sm:mt-3 sm:pt-2 border-t border-slate-300">
              <h2 className="text-xs sm:text-sm font-black text-slate-900 uppercase underline tracking-wider">
                LAPORAN REKAPITULASI & RINCIAN PENYALURAN BANTUAN SOSIAL (BANSOS)
              </h2>
              <p className="text-[11px] sm:text-xs text-slate-600 mt-0.5">
                Periode: {new Date().toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })} &bull; Dicetak: {formatTanggalIndo(new Date().toISOString().slice(0, 10))}
              </p>
            </div>
          </div>
        ...
        {/* Modal Footer (Hidden on Print) - Ramping, tombol Cetak & PDF cukup di bagian atas */}
        <div className="p-3 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs shrink-0 print:hidden">
          <div className="flex items-center gap-2 text-slate-500 text-[11px]">
            <HeartHandshake className="w-4 h-4 text-pink-600 shrink-0" />
            <span className="truncate">
              Laporan resmi penyaluran bantuan sosial & santunan terintegrasi RW 018.
            </span>
          </div>
          <div className="w-full sm:w-auto flex justify-end">
            <button
              type="button"
              onClick={onClose}
              className="w-full sm:w-auto px-4 py-2 rounded-xl bg-slate-200 hover:bg-slate-300 active:bg-slate-400 text-slate-800 font-bold cursor-pointer transition-colors text-xs text-center"
            >
              Tutup
            </button>
          </div>
