...
export type BansosType = 'PKH' | 'BPNT (Sembako)' | 'BLT Dana Desa' | 'Beras CBP 10kg' | 'KIS / PBI' | 'PIP Sekolah' | 'Santunan Kas RW' | 'Santunan Yatim/Piatu' | 'Bedah Rumah';
...
export interface AppUser {
  id: string;
  username: string;
  /** Legacy UI field only. Passwords are managed exclusively by Firebase Authentication and are never persisted. */
  password?: string;
  firebaseUid?: string;
  nama: string;
  role: UserRole;
