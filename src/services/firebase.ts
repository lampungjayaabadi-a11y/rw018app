...
export function subscribeToCollection<T>(
  ...
  try {
    ...
    return onSnapshot(
      ...
      (error) => {
        console.warn(`[Firestore Realtime] Subscription issue for ${collectionName}:`, error?.message || error);
        if (onError) onError(error instanceof Error ? error : new Error(String(error)));
      }
    );
  } catch (error) {
    console.warn(`[Firestore Realtime] Failed to initialize subscription for ${collectionName}:`, error);
    return () => {};
  }
}
...
export function subscribeToDocument<T>(
  ...
  try {
    ...
    return onSnapshot(
      ...
      (error) => {
        console.warn(`[Firestore Realtime] Document subscription issue for ${collectionName}/${docId}:`, error?.message || error);
        if (onError) onError(error instanceof Error ? error : new Error(String(error)));
      }
    );
  } catch (error) {
    console.warn(`[Firestore Realtime] Failed to initialize document subscription for ${collectionName}/${docId}:`, error);
    return () => {};
  }
}
