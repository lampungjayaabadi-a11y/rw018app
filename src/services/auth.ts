...
export function initUsersFirestoreSync() {
  ...
  onAuthStateChanged(auth, async (firebaseUser) => {
    ...
    try {
      ...
      if (!snap.exists()) {
        console.error('[Auth] Firebase account has no profile document:', firebaseUser.uid);
        await signOut(auth);
        return;
      }
      ...
      saveUsersListLocally([user, ...getUsersList().filter((u) => u.id !== user.id)]);
      setCurrentUser(user);
    } catch (error) {
      console.error('[Auth] Failed to hydrate Firebase session:', error);
      await signOut(auth).catch(() => {});
      setCurrentUser(null);
    }
  });

  // Role permissions are configuration data. It is safe to cache them locally after authentication.
...
export function saveUsersList(users: AppUser[]): void {
  ...
  // Only Firebase UID-backed profiles are synchronized. Legacy seed IDs are local metadata only.
  users.forEach((u) => {
    if (!u.id || u.id.startsWith('usr-') || u.id.startsWith('draft_')) return;
    const { password: _password, ...safeUser } = u as any;
    saveDocumentOnline('users', u.id, safeUser).catch((error) =>
      console.warn('[Auth] Could not sync user profile:', error)
    );
  });
}
...
export async function loginUser(
  ...
  const foundUser = getUsersList().find((u) => u.username.toLowerCase() === trimmedUser)
    || initialUsers.find((u) => u.username.toLowerCase() === trimmedUser);
  ...
  if (!inputPass) return { success: false, message: 'Silakan masukkan kata sandi.' };

  // Firebase Authentication is now the only password authority. No password is stored in Firestore/localStorage.
  const email = foundUser.email || `${foundUser.username}@rw018app.com`;
  try {
    const credential = await signInWithEmailAndPassword(auth, email, inputPass);
    const profileSnap = await getDoc(doc(db, 'users', credential.user.uid));
    if (!profileSnap.exists()) {
      await signOut(auth);
      return { success: false, message: 'Akun Firebase sudah ada, tetapi profil pengguna belum dimigrasikan.' };
    }
    ...
    await saveDocumentOnline('users', credential.user.uid, { lastLogin: user.lastLogin });
    setCurrentUser(user);
    ...
    const message =
      code === 'auth/invalid-credential' || code === 'auth/wrong-password'
        ? 'Username atau kata sandi Firebase salah.'
        : code === 'auth/user-not-found'
          ? 'Akun Firebase belum dibuat. Jalankan migrasi akun terlebih dahulu.'
...
export async function loginWithBiometric(
  _username: string,
  _biometricType: 'fingerprint' | 'face' = 'fingerprint'
): Promise<{ success: boolean; user?: AppUser; message?: string }> {
  // Browser biometrics in this app are UI-level capabilities, not Firebase credentials.
  // Never use a local biometric flag as a substitute for Firebase Authentication.
  return {
    success: false,
    message: 'Login biometrik sementara dinonaktifkan demi keamanan. Gunakan akun Firebase Authentication.'
  };
}
...
export async function logoutUser(): Promise<void> {
  try { await signOut(auth); } catch (error) { console.warn('[Auth] Firebase signOut failed:', error); }
  setCurrentUser(null);
}
...
export function saveUserAccount(user: AppUser): void {
  ...
  saveDocumentOnline('users', user.id, safeUser);
...
export function resetUsersToDefault(): void {
  // Reset only the local selector metadata. Firebase Auth accounts and server-side roles
  // must never be overwritten by a client-side seed reset.
  saveUsersListLocally(initialUsers);
  const current = getCurrentUser();
